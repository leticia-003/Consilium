...
export class ClientDashboardComponent implements OnInit {
  ...
  loadProcesses() {
    console.log('ClientDashboard: Loading processes...');
    const userId = this.auth.getUserId();
    console.log('ClientDashboard: UserId derived from token:', userId);

    if (!userId) {
      console.error('ClientDashboard: No user ID found in token.');
      this.loading = false;
      return;
    }

    this.processService.getProcessesByClient(userId).subscribe({
      next: (res) => {
        console.log('ClientDashboard: API Response:', res);
  ...
  get activeProcessesCount() {
    return this.processes.filter((p) => !p.closedAt).length;
  }
}
